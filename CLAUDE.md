...
- **임시 이미지를 무엇으로 교체할 것인가** — 직접 촬영할지, 매장 동의를 받을지 정하지
  않았다. **공개 전에 정해야 한다.**
- **`data/crawling/`을 어떻게 할 것인가** — 08-24에 검색 결과 URL을 모으려고 만든 빈
  폴더다. `.gitignore`에도 없다. 「크롤링 금지」가 카카오맵 API 응답 저장을 가리키는
  것이라면 그 구분을 `docs/mvp-decisions.md`에 적어야 하고, 아니면 폴더를 지운다.
