...
### 현재 코드는 아직 기준을 따르지 않는다

`DESIGN.md`가 코드보다 나중에 쓰였다. `app/globals.css`에는 Pretendard도
`primary (#88484a)`도 없고, 마커는 48px(기준 46px), 상세 패널은 360px(기준 400px)다.
좌측 패널·검색바는 아예 없다. **새로 만지는 부분부터 기준에 맞추고**, 전체를 한 번에
맞추는 작업은 따로 잡는다.

⚠️ `DESIGN.md`는 카카오 로그인과 북마크를 전제로 쓰여 있는데, `mvp-decisions.md` 3절은
**로그인을 스코프에서 명시적으로 제외**했다(콜드스타트 문제를 만들지 않으려고).
둘 중 하나가 틀렸다. **로그인 관련 UI를 만들기 전에 어느 쪽을 살릴지 정해야 한다.**
