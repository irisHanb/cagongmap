...
4. 카카오 로그인 또는 로그인 사용자 정보
5. `카페 제보하기` 진입점
6. 로그인 후 북마크 패널
7. 카페 전체 목록 (검색 입력칸 포함)

제거된 요소:

- ~~후보 카페 리스트~~ — **2026-08-26에 7번으로 되돌렸다.** 아래 「카페 전체 목록」 참고.
- `조건 빠르게 고르기`
- 필터 chip 묶음

이 패널은 탐색 결과를 모두 보여주는 곳이 아니라, 지도 탐색을 시작하고 개인 상태로 들어가는 entrance다.

**현재 구현 범위는 1-7번 전부다.** 4번 카카오 로그인과 6번 북마크가
2026-08-15에, 5번 제보 진입점이 2026-08-20에, 7번 전체 목록이 2026-08-26에 들어왔다
(`scope.md` 변경 이력, `mvp-decisions.md` 3절).

**검색 입력칸은 7번 카페 전체 목록 상자 안, 제목과 목록 사이에 있다** (2026-08-26).
별도 Left Panel 순서를 차지하지 않는 이유는 검색이 목록 없이 단독으로 쓰이지 않기
때문이다 — 이름·주소 부분 일치로 목록과 지도 마커를 함께 거른다 (`lib/cafe-search.ts`,
Interaction Rules).

3번의 `목록으로 보기`는 2026-08-21에 붙었다. `/cafes`로 내보내는 텍스트 링크이고,
7번이 생긴 뒤에도 남는다 — 저쪽은 크롤러와 LLM이 읽는 서버 렌더 문서라 목적이 다르다.
보조 설명 끝에 작게 붙이고 CTA로 키우지 않는다. dock에 큰 버튼은 카카오 로그인 하나뿐이어야 한다.

지금 순서는 브랜드 블록 → 로그인 → 제보 → 북마크 패널 → 전체 목록(검색 포함)이다.

5번 제보는 **로그인 전에도 보인다.** 상세의 하트와 같은 이유다 — 제보할 수 있다는
사실 자체가 로그인의 이유이므로 그 입구까지 감추면 로그인할 까닭이 줄어든다.
누르면 폼 대신 로그인 모달이 뜬다.

### 카페 전체 목록 (Left Panel 7번)
...
- **제목 아래, 목록 위에 검색 입력칸이 있다** (2026-08-26). placeholder는
  `동네나 카페 이름`이고, 규격은 Search Bar 컴포넌트를 그대로 쓴다. 이름·주소
  부분 일치로 목록과 지도 마커를 함께 거른다 — 필터 판정은 `lib/cafe-search.ts`
  한 곳에만 있다. 결과가 0곳이면 카드 대신 한 줄 문구를 보여준다 (빈 상태를 숨기지
  않는다).
...
**자리는 Left Panel 7번 카페 전체 목록 상자 안이다** (2026-08-26). 별도 순서를 차지하지
않는다 — 검색어가 그 목록과 지도 마커를 함께 거르므로 목록과 떨어뜨려 둘 이유가 없다.
필터·정렬 조건 UI는 여전히 만들지 않는다. 이름·주소 부분 일치만 본다.
...
상세의 리뷰는 같은 크기 chip 세 개다. 높이 44px, pill radius, 가로를 균등하게 나눈다.

**chip에 글자를 적지 않는다** (2026-08-26 변경). `좋아요`·`보통`·`별로` 대신 22px 이모지
(😊 · 😐 · 😕)와 그 값의 집계 숫자(15px)를 나란히 둔다. 세 값이 같은 종류의 표정이라
눈이 바로 비교하고, 숫자가 chip 위로 올라와 몇 명이 눌렀는지가 먼저 읽힌다. 말은
`aria-label`로 남아 스크린리더가 여전히 `좋아요`로 부른다. 바뀌기 전후 화면은
`docs/review-chips/before.png` · `after.png`에 있다.
...
chip 아래 한 줄은 **전체 개수**다. 13px 회색이고 bold다.

- 값별 숫자는 chip이 이미 들고 있으므로 이 줄은 `전체 3개`처럼 합계만 말한다.
- 평가가 없으면 `전체 0개` 대신 `아직 평가가 없어요. 첫 평가를 남겨보세요`로 적는다.
  카페 9곳 대부분이 그 상태이고, `0`은 정보가 아니라 소음이다.
- chip의 숫자는 셋을 모두 적는다. `별로`가 0이어도 감추지 않는다.
  **좋은 것만 보여주면 집계가 아니라 광고가 된다.**
- 조회가 끝나기 전에는 숫자를 말하지 않되 chip 안의 자리와 줄 높이는 비워 둔다.
  나중에 끼어들며 아래를 밀어내지 않게 한다.
...
- 검색어는 마커 표시 범위를 줄인다. 이름·주소 부분 일치이고, 목록과 마커가 같은 배열을 받는다.
- **검색어가 열려 있는 상세를 닫지는 않는다.** 걸러진 카페의 상세를 보던 중이면 마커와
  목록에서만 사라지고 패널은 남는다. 읽던 것을 시스템이 뺏지 않는다 — 패널은 닫기
  버튼으로 닫는다.
- **북마크 패널은 검색어에 걸러지지 않는다.** 저장 목록은 탐색 도구가 아니다.
