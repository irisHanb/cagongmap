...
- 로그인 유도는 화면당 하나다. 하트를 누르면 모달이 뜨고, dock에는 유도를 겹쳐 두지 않는다.
...
**현재 구현 범위는 1-3번, 5번, 6번이다.** 5번 카카오 로그인과 6번 북마크가
2026-08-15에 들어왔다 (`scope.md` 변경 이력, `mvp-decisions.md` 3절).

**4번 검색바만 남았다.** `scope.md`가 필터·리스트를 2차로 미뤄둔 그대로다.
동작하지 않는 검색바를 먼저 띄우지 않는다.

지금 순서는 브랜드 블록 → 로그인 → 북마크 패널이다. 4번 검색바가 나중에 들어오면
브랜드와 로그인 사이에 끼어들어야 위 순서가 맞는다.
...
7번 확인일은 이 화면의 장식이 아니라 **지켜야 하는 항목**이다. `mvp-decisions.md` 2-3이
...
모달은 로그인 안내 하나뿐이다. 새 색이나 스케일 밖 여백을 만들지 않고 기존 토큰만 쓴다.

- Scrim: `rgba(33, 27, 26, 0.42)` + `blur(3px)`
- Panel: `surface`, `{rounded.panel}`, `{spacing.lg}` padding, `{shadows.brew}`, 최대 폭 320px
- 제목 18px/800, 본문 14px `on-surface-variant`
- 액션은 카카오 로그인 버튼과 `나중에` 텍스트 버튼 둘뿐이다.

모달을 확인 대화상자로 늘리지 않는다. 북마크 해제에는 확인을 받지 않는다 — 되돌리는
비용이 한 번 더 누르는 것뿐이다.
...
- 모달은 Escape·배경 클릭·`나중에` 셋 다로 닫힌다. 막다른 골목을 만들지 않는다.
